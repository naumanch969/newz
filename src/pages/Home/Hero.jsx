import { MoreHoriz, ArrowLeft, ArrowRight } from "@mui/icons-material"
        ...
        <div className="text-white " >
            ...
            <div className="flex flex-col gap-[12px] pb-[3rem] border-b-[1px] border-gray-400 " >
                ...
                <div className="flex items-center justify-between " >
                    <div className="w-full flex items-center gap-[8px] " >
                        ...
                        <h4 className="capitalize text-white text-[20px] " >the latest</h4>
                    </div>
                    <div className="flex " >
                        <IconButton style={{background:'#242424'}} className="bg-dark-gray " ><ArrowLeft className="text-white" /></IconButton>
                        <IconButton style={{background:'#242424'}} className="bg-dark-gray " ><ArrowRight className="text-white" /></IconButton>
                    </div>
                </div>
                <div className='flex justify-between gap-[2rem] ' >
                    <div className="w-[50%] min-h-[26rem] max-h-[28rem] " >
                        <img src={article?.media} alt="" className="w-full h-full rounded-[2px]" />
                    </div>
                    <div className="w-[50%] flex flex-col justify-start gap-[14px] " >
                        <div className="flex gap-[6px] " >
                            <span>From</span>
                            <a href={article?.clean_url} className="text-blue " >{article?.clean_url}</a>
                        </div>
                        <div className="flex flex-col gap-[8px] " >
                            <h2 className="text-[40px] font-bold text-white leading-[44px] " >{article?.title}</h2>
                            <p className="text-[16px] text-white " >{article?.summary}</p>
                        </div>
                        <div className="flex items-center justify-start gap-[1rem] " >
                            <p className=" font-semibold text-[20px] text-white w-full text-end" >By <span className=" " >{article?.author}</span></p>
                            <p className="text-white ">{article?.publishedAt}</p>
                        </div>
                    </div>
            ...
            <div className="flex flex-col gap-[12px] pt-[2rem] pb-[3rem] border-b-[1px] border-gray-400 " >
                ...
                <div className="flex items-center gap-[8px] " >
                    <span className=" w-[1rem] h-[1rem] bg-blue " />
                    <h4 className="capitalize text-white-dark text-[20px] " >the uncounted</h4>
                </div>

                <div className='flex justify-between gap-[2rem] ' >
                    {
                        articles.slice(1, 3).map((news, index) => (
                            ...
                            <div key={index} className="w-[50%] " >
                                ...
                                <div className=" flex flex-col justify-start gap-[8px] " >
                                    <div className="flex flex-col gap-[8px] " >
                                        ...
                                        <p className="text-[18px] text-white " >{news?.description}</p>
                                    </div>
                                    <div className="flex items-center justify-start gap-[1rem] " >
                                        <p className=" " >by <span className="text-white-dark font-semibold  text-[20px]" >{news?.author}</span></p>
